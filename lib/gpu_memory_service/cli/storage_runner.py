...
def _run_save(args) -> None:
    ...
    logger.info(
        ...
        f"{args.output_dir} ({len({a.tensor_file for a in manifest.allocations})} shards)"
    )
    logger.info(f"Layout hash: {manifest.layout_hash}")
    logger.info(f"GDS available: {manifest.gds_available}")
...
def _run_load(args) -> None:
    ...
    logger.info(
        f"Loading GMS state: device={args.device}, socket={socket_path}, "
        f"input_dir={args.input_dir}, gds={args.gds}, "
        f"clear_existing={not args.no_clear}"
    )
    ...
    logger.info(f"Load complete: {len(id_map)} allocations committed to GMS")
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    save_p.add_argument(
        ...
        action="store_true",
        default=False,
        help="Use NIXL GDS backend for GPU-direct I/O (requires cuFile).",
    )
    save_p.add_argument(
        "--socket-path",
        type=str,
    ...
    load_p.add_argument(
        ...
        action="store_true",
        default=False,
        help="Use NIXL GDS backend for GPU-direct I/O when reading shard files.",
    )
    load_p.add_argument(
        "--socket-path",
        type=str,
