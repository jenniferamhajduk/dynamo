...
kubectl wait --for=condition=Ready \
  dynamographdeployment/qwen3-vl-agg-gb200 \
  -n "${NAMESPACE}" --timeout=600s

# Delete old benchmark pod if exists
kubectl delete pod qwen3-vl-agg-gb200-benchmark \
...
echo "    Monitor with: kubectl logs -f qwen3-vl-agg-gb200-benchmark -n ${NAMESPACE}"
